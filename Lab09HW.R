...
# Temperature
PLoss = function(RfMod=TIC05,tau=9.3,dt=1,kF=0.015,TI=5,QS=3,TR=20){
  RfMod=data.frame(date=RfMod$date,
                   Rt=RfMod$Qpred/1000,
                   Tavg=((RfMod$MaxTemp+RfMod$MinTemp)/2))
  RfMod$MF=0
  RfMod$DF=0
  
  RfMod$MF[(format(RfMod$date,"%j") %in% c(121,213,274))]=5.4*10^-4
  ...
  #dev.off() 
  #plot(RfMod$date,RfMod$MF)
  #plot(RfMod$date,RfMod$DF)
  
#PLoss from Plant-Soil Complex now
  muTS_TI=(((520-0)/5)*TI+0) 
  MS_TI=(((18.5-3.7)/5)*TI+3.7)*2000 
  
  RfMod$muS= muTS_TI*QS^((RfMod$Tavg-TR)/10)  
  RfMod$DS=(RfMod$muS*MS_TI*RfMod$Rt)/10^6         
  #plot(RfMod$date,RfMod$DS)

  RfMod
}

DPTI05=PLoss(RfMod=TIC05,tau=9.3,dt=1,kF=0.015,TI=5)
DPTI04=PLoss(RfMod=TIC04,tau=9.3,dt=1,kF=0.015,TI=4)
DPTI03=PLoss(RfMod=TIC03,tau=9.3,dt=1,kF=0.015,TI=3)
DPTI02=PLoss(RfMod=TIC02,tau=9.3,dt=1,kF=0.015,TI=2)
DPTI01=PLoss(RfMod=TIC01,tau=9.3,dt=1,kF=0.015,TI=1)

#P Losses in Base flow we calculate outside for TIC05 only
...
plot(DPLT$date,DPLT$LT, type="l",ylim=c(0,10))

#HW2
#DPLT1=DPLT #----initial conditions
#DPLT2=DPLT #----5C plus avg temperature
#DPLT3=DPLT #----tau=2 in initial condition

#Plotting
ggplot() +
  geom_line(data=DPLT1,aes(x=date, y = LT, colour="magenta")) +
  geom_line(data=DPLT2,aes(x=date, y = LT, colour="cyan")) +
  labs(x = 'Date', y = 'Dissolved P export (kg/d)')+
  scale_color_identity(name = "Legend",
                       breaks = c("magenta", "cyan"),
                       labels = c("Initial conditions", "Adjusted temperature"),
                       guide = "legend")+
  theme(text = element_text(size = 10),legend.position="top",legend.justification="right")+
  ggtitle("P export as impacted by 5C increase in temperature")
 
##HW3 plotting
#Plotting
ggplot() +
  geom_line(data=DPLT1,aes(x=date, y = LT, colour="magenta")) +
  geom_line(data=DPLT3,aes(x=date, y = LT, colour="cyan")) +
  labs(x = 'Date', y = 'Dissolved P export (kg/d)')+
  scale_color_identity(name = "Legend",
                       breaks = c("magenta", "cyan"),
                       labels = c("Initial conditions", "tau changed"),
                       guide = "legend")+
  theme(text = element_text(size = 10),legend.position="top",legend.justification="right")+
